...
def update_apt(ssh, upgrade = True, dist_upgrade = True):
    common.command(ssh, "apt-get -y update")
    if upgrade: common.command(ssh, "apt-get -y upgrade")
    if dist_upgrade: common.command(ssh, "apt-get -y dist-upgrade")

def install_apt(ssh, package):
    common.command(ssh, "apt-get -y install " + package)
